...
impl TransformExpr {
    ...
    pub fn check_size(&self, before_size: usize, after_size: usize, context: &'static str) {
        assert_eq!(before_size, self.before.len(), "Number of labels before the \"->\" must be {}, only found {} labels for {}", before_size, self.before.len(), context);
    ...
    pub fn check_same_str_type(&self, context: &'static str) {
        ...
        assert!(self.before.iter().all(|l| l.str_type == str_type), "String types before the \"->\" must be the same for {}", context);
        assert!(self.after.iter().all(|label_or_attr| label_or_attr
            .as_ref()
            .map(|l| l.str_type() == str_type)
            .unwrap_or(true)), "String types after the \"->\" must be the same for {}", context);
    }
...
fn parse(expr: &[u8]) -> Result<(Vec<Label>, Vec<Option<LabelOrAttr>>)> {
    let split_idx = expr
        .windows(2)
        .position(|w| w == b"->")
        .ok_or_else(|| Error::Parse { string: utf8(expr), context: utf8(expr), reason: "missing \"->\"" })?;
    ...
    let after = after_str
        ...
        .map(|s| {
            let s = trim_ascii_whitespace(s)
                .ok_or_else(|| Error::InvalidName { string: utf8(s), context: utf8(expr) })?;
